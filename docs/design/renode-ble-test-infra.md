...
│           └── studio_requests.py  # NEW: studio_requests.json DSL converter
│                                   #   (+ programmatic API for exotic cases)
...
Request payloads are **never hand-encoded** — and (decided in review, revising
the earlier "checked-in generator script + generated hex" model) **not
generated by per-module scripts either**. Like `scripts/lib/renode/
rpc_client.py`, payload encoding is shared infrastructure; a module ships one
...
- The case dir contains **`studio_requests.json`**: an ordered JSON array of
  `zmk.studio.Request` messages in protobuf's canonical JSON mapping, parsed
  with `google.protobuf.json_format.ParseDict` against the real compiled
  descriptors (field names/enums validated for free; any core/custom request
  is expressible without inventing schema). One DSL extension: a *bytes*
  field may be written as `{"$type": "<full.message.name>", ...fields}` —
  the library resolves the name against the compiled proto pool (workspace
  `zmk-studio-messages` protos + the module's own `proto/` tree, compiled
  automatically via `compile_protos`), encodes that message, and substitutes
  the bytes; `$type` nests recursively, and canonical base64 strings still
  work for raw payloads. `request_id` is auto-assigned (1-based array
  position) when omitted.
- The runner converts the JSON at test time (`scripts/lib/ble/
  studio_requests.py`) into the framed request list and embeds it into the
  shared app (`-DSTUDIO_REQUESTS_HEX_FILE=<derived file>` + `hex2inc.py` →
  generated `requests.inc`); the derived file lives under the case build dir
  — nothing generated is checked in. This makes python `protobuf` + `protoc`
  a runtime dependency of `west zmk-ble-test` (installed by the ble action,
  documented in requirements-test.txt).
- The runner stages the app as `<sim id>_studio_host.exe`; `siblings.txt`
...
- The app sends the requests in order (one per response), hexdumps every
  ...
  flowing.
- Escape hatches, in increasing order of power: a byte-exact
  `studio_requests.hex` case file (one hex-encoded framed Request per line;
  mutually exclusive with the `.json` — both at once is an error) for
  payloads the JSON mapping cannot express; the programmatic API in
  `studio_requests.py` (build Request protos in Python, emit the hex) for
  computed payloads; and a module-owned `tests/ble/*_host/` app via
  auto-discovery (the legacy `tests/ble/*_central/` naming is auto-discovered
  too, for backward compat) for custom host logic (arbitrary sequencing,
  custom asserts).
...
on the consumer's whole-`dependencies/` cache) → install python-protobuf + protoc
(same progressive fallbacks as the renode action; needed for the
`studio_requests.json` conversion, 3.5) → `west zmk-ble-test <tests>` with
...
3. **BLE**: `tests/ble/<group>/<case>/` with the per-case files → CI: the ble action.
   A Studio-over-BLE case additionally checks in one declarative
   `studio_requests.json` (the request sequence in protobuf's canonical JSON
   mapping, `$type` for nested-encoded bytes), referencing the shared host
   app via `{studio_host}` in `siblings.txt` — no protobuf hand-encoding, no
   Python, no C at all.
...
- **Phase 2 — BLE into zmk-west-commands**
  1. This repo: `west zmk-ble-test` + runner + ble action + the shared
     `ble-studio-host/` app with per-case `studio_requests.json` injection
     (3.5; test-time conversion, protobuf/protoc installed by the action).
  2. Template PR #49: replace `tests/ble/run-ble-test.sh` with the command, switch
     ...
     with a `studio_requests.json` (or keep it under the auto-discovered
     `*_host` escape hatch — `studio_rpc_central/`'s existing `_central`
     naming is still auto-discovered too, so it can also stay unchanged),
     `test.py`'s `test_ble` shells out to `west zmk-ble-test`.
- **Phase 3 (optional)** — `--build` convenience for `zmk-renode-test`; parallel
  case execution by default. (The earlier "CI freshness check for generated
  hex" item is obsolete: `studio_requests.json` is the single source of truth
  and is converted at test time — nothing generated is checked in.)
...
- **Python protobuf** becomes a runtime dependency of `zmk-renode-test` (proto
  compilation) and of `zmk-ble-test` (the `studio_requests.json` conversion,
  3.5). Shipped as `requirements-test.txt`; the commands fail with an
  actionable message when missing; both actions install it.
