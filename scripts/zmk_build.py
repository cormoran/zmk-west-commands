...
class ZMKBuild(WestCommand):
    ...
    def do_run(self, args, unknown_args):
        ...
        build_yaml = self._load_yaml(args.build_yaml) if args.build_yaml else self._find_build_yaml(Path(zmk_config))
    ...
    def _load_yaml(self, path: Path | str) -> dict:
        with open(path, 'r') as f:
            ...
            res =  {
                "boards":  list(itertools.chain.from_iterable(map(lambda y: y.get('board', []), yamls))),
                "shields": list(itertools.chain.from_iterable(map(lambda y: y.get('shield', []), yamls))),
                "include": list(itertools.chain.from_iterable(map(lambda y: y.get('include', []), yamls)))
            }
    ...
    def _run_for_all(self, zmk, manifest, args, build_yaml: dict):
        ...
        for i, inc in enumerate(matrix):
            if 'artifact' not in inc:
                if len(matrix) > 0:
                    ...
                    inc['artifact'] = args.artifact if args.artifact else Path(args.config_path).name
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallelism) as executor:
            futures = [
                executor.submit(self._run_single_build, id, zmk, manifest, args, inc) for id, inc in enumerate(matrix)
            ]
            code = 0
    ...
    def _run_single_build(self, id, zmk, manifest, args, build_setup):
        ...
        build_dir = Path(args.build_dir) / artifact_name if args.build_dir else Path(west_topdir()) / 'build' / artifact_name
        west_args = args.west_args
        build_cmake_args = build_setup.get('cmake-args', build_setup.get('cmake_args', ''))
        cmake_args = (args.cmake_args.split() if args.cmake_args else []) + build_cmake_args.split()
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False) as out_log:
            ...
            proc = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=10)
        ...
        if proc.returncode != 0:
            ...
            log.err(f'[{id}] Build succeeded but firmware artifact not found in {build_dir / "zephyr" / "zmk.uf2"}')
        else:
            log.inf(f'[{id}] Build succeeded. Firmware artifact at: {build_dir / "zephyr" / "zmk.uf2"}')
