...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        "GetDeviceInfo over emulated UARTs.",
    )
    ap.add_argument(
        "--host-elf",
        type=Path,
    ...
    ap.add_argument(
        "--peripheral-elf",
        type=Path,
        help="ble-split mode only: the split PERIPHERAL half's real ZMK ELF (--elf is "
        "the split CENTRAL half).",
    )
    ap.add_argument(
        "--no-rpc",
        action="store_true",
    ...
    if args.mode == "uart" and args.host_elf is not None:
        ...
        return 2
    if args.mode != "ble-split" and args.peripheral_elf is not None:
        print("--peripheral-elf is only valid with --mode ble-split", file=sys.stderr)
        return 2
